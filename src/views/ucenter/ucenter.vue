  ...
  <div class="center-view">
    ...
    >
      <img
        ...
        alt=""
      />
      <span class="label" @click="$router.push('/money')">余额</span>
...
export default {
  ...
  created() {
    if (localStorage.getItem('jwt')) {
      // 客户端存在token 发送请求获取个人信息 否则不能发送
      // 传递的token中包含的用户id信息,不需要单独传递
      this.$http
        // .get('http://localhost:3000/v1/user', {
        //   // 服务端需要校验客户端传递的token信息
        //   headers: { authorization: localStorage.getItem('jwt') }
        // })
        .get('http://47.94.17.15:3000/v1/user')
        .then((res) => {
          console.log(res)
